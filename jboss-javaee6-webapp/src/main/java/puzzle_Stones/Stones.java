...
public class Stones extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (int rows = 0; rows < console.getRows(); rows++) {
			for (int columns = 0; columns < console.getColumns(); columns++) {
				if (console.getValue(rows, columns) != 0 && !console.Solved()) {
					out.print("<a href=\"?row=" + rows + "&column=" + columns + "\" class=\"Stone\">"
							+ console.getValue(rows, columns) + "</a>");
		...
		out.printf("<a href=\"?new=true\" class=\"myButton\">New Game</a>");
		out.print("</div>");
		if (console.Solved()) {
			endTime = System.currentTimeMillis();
			startTime = (long) session.getAttribute("startTime");
			scoresServ.InitializeNameTimes("Stones", startTime, endTime);
