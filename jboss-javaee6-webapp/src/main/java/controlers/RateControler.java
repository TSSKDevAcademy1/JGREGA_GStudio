...
//		int rate = rate1;
//		rateNew.setRate(String.valueOf(rateNew.getRate()));
		System.out.println("Game: "+game+" Rate: "+rateNew.getRate()+"");
		
		if (loggeduser.getLogin() != null ) {
			try {
				rateNew.setName(loggeduser.getLogin());
				rateNew.setGame(game);
				rateNew.setRate(rateNew.getRate());
				rateBean.register(rateNew);
				FacesMessage m = new FacesMessage(FacesMessage.SEVERITY_INFO, "Rate is inputed!",
						"Thanks for Rating");
	...
	public double callRateView(String game) throws SQLException {
		return rateBean.rateView(game);
	}
